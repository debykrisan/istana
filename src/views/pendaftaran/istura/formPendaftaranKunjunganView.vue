					...
					>
						...
						<div class="input-group">
							<Datepicker
								...
								:disabled="idIstana === ''"
								:disabled-week-days="[...hariLiburIstana]"
				...
				>
					...
					>
						<span class="mb-1"
							...
							></span
						>
						<div class="input-group" v-if="idIstura">
							<input 
								type="text" 
								class="form-control"
								id="fileDaftarPeserta"
								readonly
								v-model="fileDaftarPeserta"
							>
...
const onIstanaChange = (event) => {
	...
	api.get(
		import.meta.env.VITE_APP_RESTFUL_BASE_URL +
			"/istana/get-hari-libur/" +
			id.value
	)
		.then((response) => {
			let data = response.data.data;
			hariLiburIstana.value = data.hari_libur;
			hariLiburIstana.value.replace('\\"','');
		})
		.catch(function (error) {
			console.log(error);
		});
...
onMounted(async () => {
	...
	let accessToken = dekripJwt(atob(cookie.get(import.meta.env.VITE_APP_ACCESSTOKEN)));
	idUser.value = accessToken.idUser;
...
const onSubmit = async () => {
	if (
		...
		if(Number(jumlahPeserta.value) <= Number(kuota.value)) {
...
const fetchGroup = (idIstura) => {
	...
	api.get(
		...
		.then((response) => {
			...
			fileSuratPermohonan.value = dt.file_surat_permohonan;
			
			loadFileDaftarPeserta(dt.file_peserta);
			loadFileSuratPermohonan(dt.file_surat_permohonan);
...
function loadFileDaftarPeserta(key) {
    var ext = key.split('.').pop();
    let paramForm = {
        key_gambar: key
    }
    api.post(import.meta.env.VITE_APP_RESTFUL_BASE_URL + '/istura/presignedurl', paramForm)
    .then((response) => {
        if (ext == "jpeg" || ext == "jpg" || ext == "png" || ext == "JPEG" || ext == "PNG" ) {
            filePesertaUrl.value = response.data.imgProxyUrl
        } else {
            filePesertaUrl.value = response.data.presignedUrl
        }
    })
}

function loadFileSuratPermohonan(key) {
    var ext = key.split('.').pop();
    let paramForm = {
        key_gambar: key
    }
    ...
    .then((response) => {
        if (ext == "jpeg" || ext == "jpg" || ext == "png" || ext == "JPEG" || ext == "PNG" ) {
            fileSuratUrl.value = response.data.imgProxyUrl
        } else {
            fileSuratUrl.value = response.data.presignedUrl
        }
    })
}
</script>
